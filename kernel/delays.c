...
void wait_cycles(uint32_t number_of_cycles_to_wait) {
  ...
  {
    while (number_of_cycles_to_wait--) // If the number of cycles isn't zero
                                       // decrement the
 ...
 * Finaly in a do while loop continue reading the counter until it reaches the
 * expire value
 */
void wait_usec(uint32_t usec) {
  ...
  asm volatile("mrs %0, cntpct_el0"
               : "=r"(expire_value)); // Get current value of counter
  expire_value +=
    ((frequency / 1000) * usec) / 1000; // Calculate expire value for counter
  do {
 ...
 * This is done by
 * First do two seperate 32 bit reads of the SYSTEM_TIMER_LOW and
 * SYSTEM_TIMER_HIGH Then if the high bits changed during the LOW read then we
 * have to read it again no register will change in the time it takes to read
 * them again Then join the two together to an unsigned long (64bit integer)
 */
uint64_t get_system_timer() {
  ...
  return ((uint64_t)high_part << 32) |
         low_part; // Put the value of the high_part in the
