...
static void __attribute__((unused, aligned(4096))) program1(uint64_t arg) {
  static int f = 0;
  f %= 40;
  arg = fib(f);
  printf("fib(%d) = %d\n", f++, arg);
...
static void __attribute__((unused, aligned(4096))) program2() {
  delay(0x1ffffff0);
  printf("delay\n");
}

static void __attribute__((unused, aligned(4096))) program3() {
  unsigned int i = 0;
  for (;;) {
    delay(0xcffff0);
    printf("%c", 'a' + ((i++) % ('z' - 'a' + 1)));
...
static struct trapframe *ctx_create(vaddr_t program) {
  /*prepare new thread context*/
  flags_t flags = L2_BLOCK | ATTR_SH(ATTR_SH_IS) | ATTR_NS |
                  ATTR_AP(ATTR_AP_RW) | ATTR_AF | L3_PAGE |
