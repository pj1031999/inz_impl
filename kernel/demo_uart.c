...
static void us_setup(paddr_t pt_root, vaddr_t stack) {
  ...
  reg_ttbr0_el1_write(((uint64_t)user_space.asid << TTBR_ASID_OFFSET) |
                      (uint64_t)user_space.pt_root);

  reg_sp_el0_write(stack + PAGESIZE);
}

static paddr_t us_setup_pagetable(vaddr_t *stack, vaddr_t *usstack) {

  flags_t flags = ATTR_SH(ATTR_SH_IS) | ATTR_NS | L3_PAGE | FLAG_MEM_RW |
                  FLAG_MEM_NOT_EX | ATTR_AF | ATTR_IDX(ATTR_NORMAL_MEM_NC);
  ...
  const uint64_t PTE_ATTR = ATTR_SH(ATTR_SH_IS) | ATTR_NS | L3_PAGE | ATTR_AF |
                            ATTR_IDX(ATTR_NORMAL_MEM_NC) |
                            ATTR_AP(ATTR_AP_USER);
...
static void ctx_us_switch(vaddr_t va_stack) {
